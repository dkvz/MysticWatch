...
public class JFrameTransactionLogging extends javax.swing.JFrame implements CanLogMessages, Observer {
    ...
    public JFrameTransactionLogging(JFrameMain mainFrame) {
        ...
        this.watchThread.setLogger(this);
    ...
    public final void buildComboFromFiles() {
        ...
        if (logDir.exists() && logDir.isDirectory()) {
            ...
            JOptionPane.showMessageDialog(null, "The transaction log directory (" + TPTransactionLog.PATH_TRANSACTION_LOG
                    + ") iJOptions NOT a directory or the directory creation failed.\nYou'll have to manually"
    ...
    private void jButtonClearLogActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonClearLogActionPerformed
        if (this.currentlyDisplayed != null) {
            int dialogResult = JOptionPane.showConfirmDialog(null, "This will clear all the data in the file, proceed?", "Clear TP transaction log", JOptionPane.YES_NO_OPTION);
            if (dialogResult == JOptionPane.YES_OPTION) {
                String base = TPTransactionLog.PATH_TRANSACTION_LOG.concat(File.separator).concat(Long.toString(this.currentlyDisplayed.getItemId()));
                File log = new File(base.concat(TPTransactionLog.LOG_EXTENSION));
                PrintWriter pw;
                try {
                    pw = new PrintWriter(log.getPath());
                    pw.close();
                    // Remove all the entries in the model:
                    this.clearDataTableModel();
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Could not clear the transaction log, it's locked by another process or the logging thread.\nPlease stop all logging and try again.", "Clear TP transaction log", JOptionPane.ERROR_MESSAGE);
                }
            }
        }
    }//GEN-LAST:event_jButtonClearLogActionPerformed

    private void clearDataTableModel() {
        ...
        if (tranLog instanceof TPTransactionLog) {
            if (this.currentlyDisplayed != null) {
                ...
                if (this.currentlyDisplayed.equals(tpLog)) {
                    this.logMessage("Adding " + tpLog.getEventListRead().size() + " events to the list.");
                    ...
                    model.addEvents(tpLog.getEventListRead());
    ...
    public void refreshDataTable() {
        ...
        //this.jTableTransactionLog.repaint();
    }
